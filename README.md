...
Le pipeline ne refait jamais un travail déjà fait : si `output/<video>/transcript.json` existe, la transcription est réutilisée ; si `candidates.json` existe, le scoring est réutilisé, etc. Pour forcer un recalcul complet, mettre `overwrite: true` dans `config.yaml` (section `pipeline`).
...
| 2 | Ingestion vidéo (fichier local / URL) | À venir |
